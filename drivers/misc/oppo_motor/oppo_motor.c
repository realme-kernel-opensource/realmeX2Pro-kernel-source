...
static void position_detect_work(struct work_struct *work)
{
	...
	while (chip->motor_started) {
		...
		/*will check hall data's monotonicity in speed down and calibration condition*/
		if ((hall0_val > chip->cali_data.irq_pos_hall0 && chip->move_state == MOTOR_DOWNWARD_ING)
			|| (hall1_val > chip->cali_data.irq_pos_hall1 && chip->move_state == MOTOR_UPWARD_ING)) {
				hall_monotonicity_check = 1;
		}

		if (chip->extre_stop_support && ((chip->is_in_calibration && hall_monotonicity_check) || speed_down)
