...
void oppo_parse_motor_info(struct oppo_motor_chip *chip)
{
    ...
    if (is_project(OPPO_18115)) {
        ...
        chip->stop.neg[1] = -4;
        chip->stop.pos[1] = 4;
    } else if (is_project(OPPO_19071)) {
        chip->pwm_param.normal.up_brake_delay = 40000;
        chip->pwm_param.normal.down_brake_delay = 25000;
        chip->stop.neg[1] = -4;
        chip->stop.pos[1] = 4;
    } else if (is_project(OPPO_19371)) {
        chip->pwm_param.normal.up_brake_delay = 40000;
        chip->pwm_param.normal.down_brake_delay = 25000;
        chip->stop.neg[1] = -4;
        chip->stop.pos[1] = 4;
    ...
    if ((MSM_BOOT_MODE__RECOVERY == get_boot_mode()) || (MSM_BOOT_MODE__FACTORY == get_boot_mode()) || qpnp_is_power_off_charging() || (MSM_BOOT_MODE__SAU == get_boot_mode()))
