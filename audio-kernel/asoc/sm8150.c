...
static bool msm_usbc_set_LR_cnt(struct snd_soc_codec *codec,
				    bool active, enum usbc_switch_type type)
{
	struct snd_soc_card *card = codec->component.card;
	struct msm_asoc_mach_data *pdata =
				snd_soc_card_get_drvdata(card);
	...
	if((FSA4480 == wcd_mbhc_cfg.switch_type) && (FSA4480 == type)) {
		if (!pdata->fsa_handle)
			return false;

		ret = fsa4480_set_LR_cnt(pdata->fsa_handle, active);
	} else if((MAX20328 == wcd_mbhc_cfg.switch_type) && (MAX20328 == type)) {
		if (!pdata->max_handle)
			return false;

		ret = max20328_set_LR_cnt(pdata->max_handle, active);
	} else {
		dev_dbg(codec->dev, "%s: switch_type = %d, set type = %d\n",
					__func__, wcd_mbhc_cfg.switch_type, type);
	}
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (wcd_mbhc_cfg.enable_usbc_analog) {
		wcd_mbhc_cfg.swap_gnd_mic = msm_usbc_swap_gnd_mic;
		wcd_mbhc_cfg.set_LR_cnt = msm_usbc_set_LR_cnt;
	}
	#endif /* VENDOR_EDIT */
