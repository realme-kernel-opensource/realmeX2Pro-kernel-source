...
int max98937_reg_channel_map[][8][2] = {
	{	//mono
		{MAX98937_Boost_Control_1, 0x40},
...
//
//Speaker DC resistance is 6.8 +/-15% (5.0 --> 8.0)
//1<<27 is 134217728, for example: 5.0 / 3.33 * 134217728 = 201528120
#define SPEAKER_RDC_MIN  (201528120)  // 5.0 / 3.33 * (1<<27)
#define SPEAKER_RDC_MAX  (322444992)  // 8.0 / 3.33 * (1<<27)
#define SPEAKER_RDC_DEFAULT (261986556)  // 6.5 / 3.33 * 134217728

//Receiver DC resistance is 13 +/-15% (5.0 --> 8.0)
//1<<27 is 134217728, for example: 5.0 / 3.33 * 134217728 = 201528120
#define EAR_RDC_MIN  (201528120)  // 5.0 / 3.33 * (1<<27)
#define EAR_RDC_MAX  (322444992)  // 8.0 / 3.33 * (1<<27)
#define EAR_RDC_DEFAULT (261986556)  // 6.5 / 3.33 * 134217728
...
static int max989xx_calib_save (uint32_t calib_value, int ch)
{
	...
	if (ch == MAX98927L) {
		if (i2c_states & MAX98927_CH0) {
			...
			pr_err("%s: ch: %d, i2c_states: %d, not match\n", __func__, ch, i2c_states);
			return -1;
		}
	} else if (ch == MAX98927R) {
		filename = CALIBRATE_FILE_R;
...
static void max98927_self_recovery(struct max989xx_priv* max98927, int ch)
{
	...
	for (i = 0; i < ARRAY_SIZE(max98937_reg_channel_map[2]); i++)
		regmap_write(max98927->regmap[ch],
				max98937_reg_channel_map[2][i][0], max98937_reg_channel_map[2][i][1]);
	...
	struct device *dev, int id)
{
    ...
    if (id == MAX98927L) {
        ...
        gpio_direction_output(max98927->reset_gpio_l, 1);
        #ifdef VENDOR_EDIT
        /*Zhao.Pan@PSW.MM.AudioDriver.SmartPA, 2019/06/13, Add delay after reset. */
        msleep(1);
        #endif /* VENDOR_EDIT */
    } else {
